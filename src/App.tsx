...
// --- Helpers ---
// Convert CMS paths (/images/...) to relative paths for base: './'
...
export default function App() {
  ...
  // Lock body scroll when a lightbox is active
  useEffect(() => {
    document.body.style.overflow = (expandedProject || selectedLabItem) ? 'hidden' : '';
  ...
  const nextSlide = () => setActiveSlide((prev) => (prev + 1) % heroData.images.length);
  const prevSlide = () => setActiveSlide((prev) => (prev === 0 ? heroData.images.length - 1 : prev - 1));
    ...
    <div className={`min-h-screen selection:bg-orange-500 selection:text-white ${isDarkMode ? 'dark' : 'light'}`}>
      ...
      <nav className="fixed top-0 w-full z-50 border-b border-gray-100 dark:border-neutral-900 bg-white/80 dark:bg-black/80 backdrop-blur-md">
        <div className="container-wide h-16 flex items-center justify-between">
          <a href="#" className="font-bold text-lg tracking-tighter text-black dark:text-white">
            {settingsData.navigation.logoText}
          </a>
        ...
        {mobileOpen && (
          ...
          >
            <div className="flex flex-col gap-6 text-base tracking-normal pt-8">
              {settingsData.navigation.links.map((link, idx) => (
                <motion.a
      ...
      <section id="home" className="relative h-screen overflow-hidden">
        <AnimatePresence mode="wait">
          ...
          >
            <img
              src={assetUrl(heroData.images[activeSlide].url)}
              alt="Digital Art"
      ...
      <section id="projects" className="py-24 md:py-32 bg-white dark:bg-black">
        <div className="container-wide">
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 mb-16">
            <div>
              <p className="text-xs uppercase tracking-widest font-bold opacity-30 mb-2">{projectsData.sectionLabel}</p>
              <h2 className="text-4xl md:text-6xl font-bold tracking-tighter">{projectsData.sectionHeading}</h2>
            </div>
      ...
      <section id="lab" className="py-24 bg-gray-50 dark:bg-neutral-950 border-y border-gray-100 dark:border-neutral-900">
        <div className="container-wide">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            <div>
              <p className="text-xs uppercase tracking-widest font-bold opacity-30 mb-2">{labData.sectionLabel}</p>
              <h2 className="text-4xl md:text-5xl font-bold tracking-tighter mb-6">{labData.heading}</h2>
              <p className="text-lg opacity-60 mb-8 max-w-lg">{labData.description}</p>
              <div className="flex flex-wrap gap-4">
                {labData.tools.map(tool => (
                  <span key={tool.name} className="px-4 py-2 bg-white dark:bg-black border border-gray-100 dark:border-neutral-800 text-xs font-mono rounded-full">
                    {tool.name}
                  </span>
                ))}
              </div>
      ...
      <AnimatePresence>
        {selectedLabItem && (
          ...
          >
            ...
            >
              <div className="aspect-video w-full overflow-hidden bg-neutral-900 rounded-xl">
                <img
                  src={assetUrl(selectedLabItem.image)}
              ...
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                ...
                <p className="text-sm text-neutral-400 max-w-md leading-relaxed">{selectedLabItem.description}</p>
              </div>
            </motion.div>
          </motion.div>
        )}
              ...
              >
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 md:gap-12 items-start">
                  <div className="lg:col-span-5 space-y-6">
                    <div>
                      <span className="font-mono text-xs text-orange-400 tracking-wider uppercase mb-2 block">{project.category}</span>
                      <h3 className="text-3xl md:text-5xl font-bold tracking-tighter leading-tight mb-4 text-white">{project.title}</h3>
                      <p className="text-sm md:text-base text-neutral-400 leading-relaxed font-light">{project.longDescription}</p>
                    </div>
                    <div className="pt-6 border-t border-neutral-800 space-y-6">
                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          ...
                          <p className="font-medium text-sm text-neutral-200">{project.year} — {project.category}</p>
                        </div>
                        <div>
                          <p className="text-[10px] uppercase font-mono tracking-widest text-neutral-500 mb-1">Artist</p>
                          <p className="font-medium text-sm text-neutral-200">{infoData.heading.split(' ')[0]} {infoData.heading.split(' ').slice(1).join(' ') ? 'Abe' : ''}</p>
                        </div>
                      </div>
                      <div className="flex flex-col sm:flex-row gap-4 pt-2">
                        ...
                        >
                          {projectsData.inquireLabel} <ExternalLink size={14} />
                        </a>
                        <a
                          href="#connect"
                          onClick={() => setExpandedProject(null)}
                        ...
                        >
                          {projectsData.simulatorLabel}
                        </a>
                      </div>
                    </div>
                  </div>
                  <div className="lg:col-span-7 space-y-6">
                    {project.detailsImages.map((img, i) => (
                      <div key={`${img.url}-${i}`} className="aspect-video bg-neutral-900 overflow-hidden rounded-xl border border-neutral-900 shadow-inner group">
                        <img
          ...
          <div className="grid grid-cols-1 md:grid-cols-12 gap-16 items-start">
            <div className="md:col-span-5 md:sticky md:top-32">
              <div className="aspect-[3/4] bg-gray-100 dark:bg-neutral-900 overflow-hidden relative group">
                <img
                  src={assetUrl(infoData.image)}
                  alt={settingsData.siteTitle}
            ...
            <div className="md:col-span-7">
              <p className="text-xs uppercase tracking-widest font-bold opacity-30 mb-2">{infoData.sectionLabel}</p>
              <h2 className="text-4xl md:text-6xl font-bold tracking-tighter mb-12 leading-[0.9]">{infoData.heading}</h2>
              <div className="space-y-6 text-xl md:text-2xl opacity-60 leading-relaxed font-light">
                {infoData.bio.map((para, i) => (
                  <p key={i}>{para.text}</p>
                ))}
              </div>
              <div className="mt-16 grid grid-cols-2 gap-8 pt-16 border-t border-gray-100 dark:border-neutral-900">
                <div>
                  <h4 className="font-bold mb-4">{infoData.focusHeading}</h4>
                  <ul className="space-y-2 text-sm opacity-50">
                    {infoData.focusAreas.map((area) => (
                      <li key={area.item}>{area.item}</li>
                    ))}
                  </ul>
                </div>
                <div>
                  <h4 className="font-bold mb-4">{infoData.exhibitionsHeading}</h4>
                  <ul className="space-y-2 text-sm opacity-50">
                    {infoData.exhibitions.map((ex) => (
                      <li key={ex.item}>{ex.item}</li>
                    ))}
                  </ul>
                </div>
              </div>
      ...
      <section id="connect" className="py-16 md:py-24 bg-black text-white relative overflow-hidden">
        ...
        <div className="container-wide relative z-10">
          <div className="max-w-4xl">
            <h2 className="text-5xl md:text-8xl font-bold tracking-tighter mb-12">{connectData.heading}</h2>
            <p className="text-xl md:text-2xl mb-16 opacity-60 leading-tight">{connectData.description}</p>
            <div className="flex flex-col md:flex-row gap-12 md:items-center">
              ...
              >
                {connectData.email} <ChevronRight size={32} strokeWidth={1} />
              </a>
              <div className="flex gap-6">
                {connectData.socials.map((social, i) => {
          ...
          <div className="mt-12 pt-6 border-t border-white/10 flex flex-col md:flex-row justify-between items-start md:items-center gap-8 opacity-40 text-xs tracking-widest uppercase text-left">
            <p>{settingsData.footer.copyright}</p>
          </div>
        </div>
      </section>
    </div>
...
function LabCard({ item, onClick }: LabCardProps) {
  return (
    <div
      onClick={onClick}
